...
class _EventDetailScreenState extends ConsumerState<EventDetailScreen> {
  ...
  void _invalidateEventDetailData() {
    ...
    ref.invalidate(publicEventQuestionsProvider(widget.eventId));
    ref.invalidate(eventFeedbacksProvider(widget.eventId));
    ref.invalidate(eventFeedbackSummaryProvider(widget.eventId));
  }
  ...
  Future<void> _refreshEventDetailData() {
    ...
    ref.invalidate(eventFeedbackSummaryProvider(widget.eventId));
    return ref
        .refresh(eventDetailProvider(widget.eventId).future)
        .then<void>((_) {});
  }

  @override
  Widget build(BuildContext context) {
    final detailState = ref.watch(eventDetailProvider(widget.eventId));
    final event =
...
class _QuestionsSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final questionsState = ref.watch(publicEventQuestionsProvider(eventId));
    ...
    return questionsState.when(
      ...
      error: (_, _) => AppErrorState(
        ...
        onRetry: () => ref.invalidate(publicEventQuestionsProvider(eventId)),
      ),
      data: (questions) {
