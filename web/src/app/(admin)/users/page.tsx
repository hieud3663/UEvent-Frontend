...
import { mockUsers, userStats } from '@/features/users/mock/mock-users';
    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="flex gap-3">
          <button 
            type="button"
            onClick={() => alert('Exporting user data as CSV...')}
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="glass-card p-5 rounded-2xl flex flex-col gap-1">
          ...
          <span className="text-2xl font-black text-on-surface">{userStats.totalUsers.toLocaleString('en-US')}</span>
        ...
        <div className="glass-card p-5 rounded-2xl flex flex-col gap-1">
          ...
          <span className="text-2xl font-black text-on-surface">{userStats.activeOrganizers}</span>
        ...
        <div className="glass-card p-5 rounded-2xl flex flex-col gap-1">
          ...
          <span className="text-2xl font-black text-amber-600">{userStats.pendingRequests}</span>
        ...
        <div className="glass-card p-5 rounded-2xl flex flex-col gap-1">
          ...
          <span className="text-2xl font-black text-emerald-500">{userStats.platformStatus}</span>
          <span className="text-xs text-slate-400 font-medium">{userStats.uptime} Uptime</span>
        </div>
      </div>
      ...
      <div className="glass-card rounded-[32px] overflow-hidden shadow-2xl shadow-black/5">
        ...
        <div className="px-6 py-4 bg-white/30 flex items-center justify-between">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
            Showing 1-10 of {userStats.totalUsers.toLocaleString('en-US')}
          </span>
          <div className="flex gap-2">
            <button className="w-8 h-8 flex items-center justify-center rounded-lg border border-white/40 text-slate-400 hover:bg-white/50 transition-all">
              <ChevronLeft className="w-[18px] h-[18px]" />
            </button>
            <button className="w-8 h-8 flex items-center justify-center rounded-lg bg-amber-500 text-white text-xs font-black">
              1
            </button>
            <button className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/50 text-xs font-bold text-slate-600 transition-all">
              2
            </button>
            <button className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/50 text-xs font-bold text-slate-600 transition-all">
              3
    ...
    <tr className="hover:bg-white/40 transition-colors group">
      <td className="px-6 py-4">
        <div className="flex items-center gap-4">
          <div
            className={cn(
              'w-10 h-10 rounded-full bg-slate-200 overflow-hidden ring-2 ring-white ring-offset-2 ring-offset-transparent',
      ...
      <td className="px-6 py-4">
        <span
          className={cn(
            ...
            isBanned ? 'bg-slate-100 text-slate-500' : roleColors[user.role]
          )}
        >
          {user.role}
        </span>
      ...
      <td className="px-6 py-4 text-right">
        <div className="flex justify-end gap-3">
          ...
          {isBanned ? (
            <button
              onClick={() => alert(`Unbanned user ${user.name}`)}
